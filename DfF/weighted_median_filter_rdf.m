function dispOut = weighted_median_filter_rdf(dispIn, imgGuide, vecDisps, r, epsilon)
...
guidedfilter_color_precompute_rdf(imgGuide, r, epsilon);

for d = 1 : numel(vecDisps)
    fprintf('%d of %d\n', d, numel(vecDisps));

    % apply guided filter to each slice
    img01 = guidedfilter_color_runfilter_rdf(double(dispIn == vecDisps(d)));
